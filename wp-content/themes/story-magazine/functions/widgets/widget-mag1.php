<?php
add_action('widgets_init', 'story_magazine_mag1_widget');

function story_magazine_mag1_widget()
{
	register_widget('story_magazine_mag1_widget');
}

class story_magazine_mag1_widget extends WP_Widget {
	
	public function __construct() {
		$widget_ops = array('classname' => 'story_magazine_mag1_widget', 'description' => esc_html__('Featured posts widget.','story-magazine'));
       	parent::__construct('story_magazine_mag_1', esc_html__('wpmasters - "Home 1"','story-magazine'),$widget_ops);  
	}
		...
		?>
            ...
            <div class="mag-block mag-one">
                ...
                if($counter < 2):
					...
					get_template_part('/post-layouts/home-mag1-big');
	...
	{
		$defaults = array('title' => 'Recent Posts', 'post_type' => 'all', 'categories' => 'all', 'posts' => 4, 'show_excerpt' => null,'offset_posts' => '');
		...
		<p>
			...
			<select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories">
				<option value='all' <?php if ('all' == $instance['categories']) echo 'selected="selected"'; ?>><?php esc_html_e('all categories','story-magazine'); ?></option>
